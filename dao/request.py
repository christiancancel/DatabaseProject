...
class RequestDAO:
    ...
    def browseResourcesRequestedByr_id(self, r_id):
        cursor = self.conn.cursor()
        query = "select * from request natural inner join pin where r_id = %s order by r_pname;"
    ...
    def browseResourcesRequestedByr_pname(self, r_pname):
        cursor = self.conn.cursor()
        query = "select * from request natural inner join pin where r_pname = %s order by r_pname;"
    ...
    def browseResourcesRequestedByDate(self, r_date):
        cursor = self.conn.cursor()
        query = "select * from request natural inner join pin where r_date = %s order by r_pname;"
    ...
    def browseResourcesRequestedByQty(self, r_qty):
        cursor = self.conn.cursor()
        query = "select * from request natural inner join pin where r_qty = %s order by r_pname;"
    ...
    def browseResourcesRequestedByNameAndDate(self, r_pname, r_date):
        cursor = self.conn.cursor()
        query = "select * from request natural inner join pin where r_pname = %s and r_date = %s order by r_pname;"
    ...
    def browseResourcesAvailable(self):
        cursor = self.conn.cursor()
        query = "select * from product where p_qty > 0;"
        cursor.execute(query)
        result = []
        for row in cursor:
            result.append(row)
